...
**Status:** Phases 0–3, 6, 7, 9 complete. Full-stack tool: FastAPI backend, React + OpenLayers frontend, multi-parcel workspace, scoring engine. Works for any parcel in any US jurisdiction.
...
# Open http://localhost:8000  (serves the production build of the UI)
```

**Frontend dev** (hot reload, proxies `/v1` to `:8000`):
...
**Build frontend for production** (output goes to `app/static/`):
...
├── frontend/                # React + MUI + OpenLayers UI (Vite 5)
│   ├── src/
│   │   ├── App.jsx          # Root component — sidebar layout, parcel/draw/submit wiring
│   │   ├── api.js           # Typed wrappers for /v1/parse/* and /v1/feasibility
│   │   ├── config.js        # Display config: verdict labels, score colors, sub-score names
│   │   ├── theme.js         # MUI theme overrides (global button/input defaults)
│   │   ├── components/
│   │   │   ├── shared.jsx         # CollapsibleSection, SectionLabel, StatRow, StatusChip
│   │   │   ├── MapView.jsx        # OpenLayers map: parcel/edge layers, draw, modify
│   │   │   ├── UploadPanel.jsx    # Upload / Draw buttons
│   │   │   ├── ParcelListPanel.jsx # Parcel list with score badge, edit, delete
│   │   │   ├── EdgePanel.jsx      # Road-facing edge selection list
│   │   │   ├── ZoningPanel.jsx    # Zoning rules form (Autocomplete presets, validation)
│   │   │   └── ResultsPanel.jsx   # Verdict card, sub-scores, scenarios table
│   │   └── hooks/
│   │       ├── useParcels.js      # Per-parcel state collection (add/update/remove/active)
│   │       └── useMapLayers.js    # OL layer builders and parcel-to-map sync helpers
│   ├── eslint.config.js     # ESLint 9 flat config (react-hooks, react-refresh)
│   ├── vite.config.js       # base: '/static/', proxies /v1 to :8000 in dev
│   └── package.json
├── app/
...
│   ├── scoring/             # 0–100 score + verdict (rule-based, post-engine)
...
│   │       └── feasibility.py  # POST /v1/feasibility
│   ├── static/              # Built frontend assets — generated by `npm run build`, do not edit
...
├── tests/
│   ├── engine/              # Unit tests — no DB, no network
│   ├── parsers/             # Parser + projection tests
│   ├── api/                 # Endpoint tests — DB mocked
│   ├── adapters/            # Adapter tests — HTTP mocked, no DB
│   ├── models/              # Model structure tests
│   └── fixtures/            # Synthetic parcel/zoning fixtures
├── scripts/
...
├── .pre-commit-config.yaml  # ruff + ruff-format + eslint hooks
...
| 1 | ✅ Complete | Core feasibility engine + SQLAlchemy models, 88 tests |
| 2 | ✅ Complete | Generic ArcGIS parcel adapter (optional path), Alembic migrations |
| 3 | ✅ Complete | Geometry parsers (GeoJSON/KML/SHP), projection, FastAPI endpoints, 171 tests |
| 4 | Pending | Report persistence (reports table → Postgres) |
| 5 | Pending | Report rendering (HTML + PDF) |
| 6 | ✅ Complete | React + MUI + OpenLayers UI — file upload, draw, edge selection, zoning form, results |
| 7 | ✅ Complete | 0–100 scoring engine, verdict card, sub-score breakdown |
| 8 | Pending | Pilot validation (50–100 real parcels) |
| 9 | ✅ Complete | Multi-parcel workspace — multi-upload/draw, per-parcel state, parcel list, shape editing |
...
| Frontend | Vite 5 + React 18 + MUI v5 |
| Map | OpenLayers 9.x (parcel/edge layers, draw, vertex edit) |
| Linting | ESLint 9 (JS) + ruff (Python) enforced via pre-commit |
